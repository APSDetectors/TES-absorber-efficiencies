...
plot(energy,Sn_absorption, color="black", linewidth=2.5, linestyle="-")
plot(energy,Bi_absorption, color="red", linewidth=2.5, linestyle="-")
plot(energy,Ge_absorption, color="green", linewidth=2.5, linestyle="-")
...
xlabel('Energy (keV)', fontsize = 18)
ylabel('Absorption', fontsize = 18)
title('X-ray Absorber Quantum Efficiency', fontsize = 20)
legend((r'Sn (%d microns)'%thickness, r'Bi (%d microns)'%thickness, r'Germanium (3 mm)'), shadow = True, loc = 0,numpoints = 2)
ltext = gca().get_legend().get_texts()
setp(ltext[0], fontsize = 19, color = 'k')
setp(ltext[1], fontsize = 19, color = 'r')
setp(ltext[2], fontsize = 19, color = 'g')
